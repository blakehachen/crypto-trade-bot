...
class Rest(ABC):
    ...
    def query(self, method: str = 'get', data=None, headers=None, iri: str = None):
        ...
        if data is None:
            data = {}
        print(self.build_url(self.resource_name, iri))
        #print(f'{data} \n\n {headers}')
        response = self.client.request(method, self.build_url(self.resource_name, iri), data=data, headers=headers).json()
        #print(f'HERE WE GO AGAIN {response}')
    ...
    def post(self, data=None, headers=None):
        ...
        if data is None:
            data = {}
        #print(json.dumps(self.serialize(data)))
    ...
    def serialize(self, data=None, filters=None):
        ...
        for key, value in filtered_data.items():
            if key in self.relations and type(value) == str and '/' not in value:
                ...
                #print(filtered_data[key])
        #print('END OF SERIALIZING',filtered_data)
